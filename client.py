...
def print_room_options(room):
    ...
    room_options = "\tRoom Menu:" \
                   "\t<o> : show Top menu" \
                   "\t<r> : show list of currently joined rooms" \
                   "\t<u> : list users in room" \
                   "\t<dm> : direct message multiple rooms" \
                   "\t<n> : switch to next room"
...
def chat_in_room(room, display_name):
    global client_socket

    print_room_options(room)

    message_package = {"room": room, "message": ""}
    while True:
...
def contact_server(room):
    if room[1] == "join":
        chat_in_room(room[0], display_name)
    elif room[1] == "list":
        get_rooms_list()
...
while room[1] != "disconnect":
    ...
    room = process_top_option(option)
    contact_server(room)
